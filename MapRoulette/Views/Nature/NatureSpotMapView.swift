                ...
                ForEach(NatureSpotType.allCases, id: \.self) { spotType in
                    ...
                    if !typeSpots.isEmpty {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            .padding(.top, spotType == NatureSpotType.allCases.first ? (80 + (isIntegrated ? mapModeBandHeight + extraTopInset : 0)) : 0)
...
struct NatureSpotDetailView: View {
    ...
    /// このスポットの同梱写真（CC0 / CC BY・無ければ nil）。
    /// ある場合はヘッダー全面に敷き、無い場合は従来通りカラーヘッダーのまま
    /// （観光スポット詳細 AttractionDetailView と同じ扱い）。
    private var photo: Image? { NaturePhoto.image(for: spot.nameKey) }
